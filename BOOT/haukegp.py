...
START_CAPTURE = "http://"+GOPRO_IP+"/gp/gpControl/command/shutter?p=1"
STOP_CAPTURE = "http://"+GOPRO_IP+"/gp/gpControl/command/shutter?p=0"

START_CAM = "http://"+GOPRO_IP+"/gp/gpControl/setting/144/12"
SHOW_MEDIA = "http://"+GOPRO_IP+"/gp/gpMediaList"

capture = False
parser = argparse.ArgumentParser(description="GoPro Handler")
parser.add_argument("--UDPIP", "-i", type=str, help="UDP IP Adresse für den Stream")
parser.add_argument("--UDPPORT", "-p", type=int, help="UDP Port für den Stream")
parser.add_argument("--capture", "-c", action="store_true", help="Controlbefehl zum starten einer Aufnahme")
...
if not args.capture:
    ...
    requests.get(STOP_URL)
    requests.get(START_CAM)
    print(requests.get(START_CAPTURE))
    def exit_handler():
        print('\nStopping Capture...\n')
        print(requests.get(STOP_CAPTURE))
        print(requests.get(START_URL))
        print(requests.get(SHOW_MEDIA))
